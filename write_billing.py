...
export default function BillingPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const bizWithBilling = await Promise.all(bizData.map(async (biz: any) => {
        try {
          const billRes = await fetch(\/api/v1/businesses/\/billing/status\, {
            headers: { Authorization: \Bearer \\ }
          });
          if (billRes.ok) {
  ...
  const handleUpgrade = async (businessId: string, plan: string) => {
    ...
    try {
      const res = await fetch(\/api/v1/businesses/\/billing/upgrade\, {
        method: "POST",
        headers: {
...
'''
with open("frontend/src/app/dashboard/billing/page.tsx", "w", encoding="utf-8") as f:
    f.write(code)
